########################################################################
###################### VER 9.2 -  CALIBRATION ##########################
# YOU HAVE TO RUN VER 9.2 FIRST LESS THE DEF OF SUMPOL GETS JUNGLED UP #
########################################################################
...
pvec = 5000:5000:50000;

pvec = [7 11 13 17 21 25 29 35] 
results = zeros(15,length(pvec));
...
function parameters()
    ...
    beta::Float64 = 0.96
    delta::Float64 = 0.06
    pdef_exo::Float64 = 0.06
    discount::Float64 = beta
    phi_a::Float64 = 0.4
    tauchen_sd::Float64 = 4.0

    kappa::Float64 = 0.3          
    zeta_R::Float64 = 15000.0     # prly gonna need a larger value
...
function FirmOptim(wage; phi_c)
    ...
    SumPol::Array{Float64, 2} = zeros(n, 20)
            ...
            for a_i in 1:m
                ...
                for next_e_i in 1:e_size
                    ...
                    if x_next < x_grid[end] && x_next > x_grid[1]
                        ...
                        Pi_reo += p_trans * max(phi_c*val, 0)
...
results[:,iter_a] = vcat(totalmass, exitmass, entrymass, exitshare, YtoL, meanL, meanK, meanY, avg_b2a, avg_q, avg_gam, avg_prod, CFshare, avg_CFL, SMEshare)
