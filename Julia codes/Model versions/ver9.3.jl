###########################################################################
############### VER 9.3 - Optimal liquidation decision  ###################
########### Endogenous defaults after a lull in production ################
###########################################################################
...
function parameters()
    ...
    nu::Float64 = 2/3 * DRS
    pc::Float64 =  26.317612625812508
    beta::Float64 = 0.975
    delta::Float64 = 0.05
    pdef_exo::Float64 = 0.034630917326609646
    discount::Float64 = beta
    phi_a::Float64 = 0.4
    tauchen_sd::Float64 = 2.5
...
function FirmOptim(wage; phi_c)
    ...
    for a_i in 1:m
        for  s_i in 1:n 
            ...
            e_i = s_i_vals[s_i, 2]  # enough to save e, current x does not matter, since there are no financial frictions
            ...
            for next_e_i in 1:e_size
                if def == 0 && next_def == 0 && next_exit == 0
                    for temp_e in [e_vals[next_e_i], 0]  # Iterate over actual and "temporary lull" cases

                        # Probability of transition
                        p_trans = temp_e != 0 ? e_chain.p[e_i, next_e_i] * (1 - pdef_exo) : (pdef_exo / e_size)

                        # next period cash on hand x'(b',k',e')
                        x_next = fn_X(k, b, temp_e)
                        # Where x falls on the grid - closer
                        x_close = argmin(abs.(x_next .- x_grid))
            
                        # Handle cases for x_close and x_far
    ...
    while !isequal(kbexq_old,kbexq_new)
        ...
        for s_i in 1:n
            ...
            for a_i in 1:m
                ...
                for next_e_i in 1:e_size
                    for temp_e in [e_vals[next_e_i], 0]  # the last state corresponds to the lull

                        # Probability of transition
                        p_trans = temp_e != 0 ? e_chain.p[e_i, next_e_i] * (1 - pdef_exo) : (pdef_exo / e_size)

                        x_next = fn_X(next_k, next_b, temp_e)
                        ...
                        if x_next < x_grid[end] && x_next > x_grid[1]
                            ...
                            gam += p_trans * ( fn_Gam(next_k, val) * next_def_close * close_weight  +  fn_Gam(next_k, val) * next_def_far * (1-close_weight)) 
                            Pi_reo += p_trans * phi_c*val
        
                        else # close_weight = 1
                            val = val_close
                            pdef += p_trans * next_def_close          
                            gam += p_trans * fn_Gam(next_k, val) * next_def_close
                            Pi_reo += p_trans * phi_c*val    
                        end  
                ...
                # correction of gamma, since the above definition of gam is P(A and B)
                # using: P(A|B) = P(A & B) / P(B)
                gam = pdef != 0 ? gam / pdef : 0
    ...
    for s_i in 1:(n-1)
        ...
        for next_e_i in 1:e_size
            for temp_e in [e_vals[next_e_i], 0] 
...
############ Results: ABL vs CFL  ##############
wage = 1;
@elapsed SumPol, e_chain, Fmat = FirmOptim(wage, phi_c = 0.5)
