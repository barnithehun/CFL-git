###########################################################################
############### VER 9.1 - Optimal liquidation decision  ###################
...
function FirmOptim(wage; phi_c)
    ...
    rho_e, sigma_e, nul_e, alpha, nu, pc, beta, delta, pdef_exo, discount, phi_a, tauchen_sd, kappa, zeta_Rs, zeta_Rl, zeta_L, tau_vec, phi_c_hh, Fcut = parameters()
    ...
    fn_Gam(k,val, zeta_R) = Int( 0 >= ((1-phi_c_hh)*val - zeta_R)) # optimal liquidation
            ...
            for a_i in 1:m
                ...
                for next_e_i in 1:e_size
                    ...
                    if x_next < x_grid[end] && x_next > x_grid[1]
                        ...
                        gam += p_trans * fn_Gam(next_k, val, zeta_R) 
    ...
    ### Incumbent dynamics ###  - currently it does not take into account exo defaults!!! although it would not change much
    ...
    Fmat = Fmat .* (1 .- (SumPol[1:end-1, 8] .+ pdef_exo)) #this already takes into account def endo and def exo
    Fmat = hcat(Fmat, SumPol[1:end-1, 8] .+ pdef_exo)
...
############ Results: ABL vs CFL  ##############
wage = 1
@elapsed SumPol, e_chain, Fmat = FirmOptim(wage, phi_c =  0.25)
...
initvec = [26, 0.035, 3600, 1800, 64, 0.5]
