###########################################################################
##############################   VER 11.1  ################################
####### VER 10.2 -  but reorganization firm dynamics is fixed   ###########
###########################################################################

# Results are marginally worse than in ver 10.2, but now reorganization costs affect firm dynamics
# Hence, AB case with this code has shows some real affects that are sufficiently low
# The effect of the reform on wages is less pronounced (bit under 1%) 
#        - this seems to be the effect of the recalibration rather than the updated firm dynamics
# Results are not on the paper yet, since I probably still need to calibrate $kappa$
...
function parameters()
    ...
    nu::Float64 = 2/3 * DRS
    pc::Float64 =   32.90842302967473
    beta::Float64 = 0.96
    delta::Float64 = 0.1
    pdef_exo_l::Float64 = 0.024143735334416286
    ...
    zeta_L::Float64 = 0
    phi_c_hh::Float64 =   0.8159858443919055 # (1 - phi_r in the paper) 
    Fcut::Float64 = 1200
...
# calibration values
wage = 1;
phi_c = 0.2813862585142178
zeta_Rl = 2700.3458709237057
...
zeta_Rl = 1295
@elapsed SumPol0, e_chain0, Fmat0 = FirmOptim(1.0097, phi_c, zeta_Rl)
...
zeta_Rl = 1350
@elapsed wage_alt = find_zero(wage -> FindWage(wage, phi_c, zeta_Rl) - c_e_baseline, (1, 1.01), Bisection(), rtol=tolerance, verbose=true)

# from manual testing, wage alt should be around 1.0095
...
initvec = [ 30.2019, 0.02485, 2590.7608, 0.7960, 0.2810]
