#################################################################################
##################### Experimental model - VER 10.3  ############################
########################### 3 state variables ###################################
#################################################################################

using LinearAlgebra, Statistics, LaTeXStrings, Plots, QuantEcon, Roots, NamedArrays, SparseArrays, Dates, XLSX, DataFrames, Distributions, Random, SparseArrays
#=
...
function gridsize()
    ...
    k_size::Int = 30
    b_size::Int = 30
    return (e_size = e_size, k_size = k_size, b_size = b_size)
end
...
# function FirmOptim(wage; phi_c)
    ...
    # Setting the state-space
        ...
        s_vals = [gridmake(k_grid, b_grid, e_vals) zeros(n)]           
        s_vals = [s_vals; [0 0 0 1]]

        s_i_vals = [gridmake(1:k_size, 1:b_size, 1:e_size) zeros(n)] 
        s_i_vals = Int.([s_i_vals; [1 div(b_size,2) 1 1]])  # productivity after default does not matter
    ...
    s_indices = Vector{Int64}(undef, m * n)  # Preallocate array for state indices
    a_indices = Vector{Int64}(undef, m * n)  # Preallocate array for action indices
    Q_vals = Vector{Float64}(undef, m * n)   # Preallocate array for Q matrix values
    R_vals = Vector{Float64}(undef, m * n)   # Preallocate array for R values (if needed)
    
    index = 1  # Initialize an index to track the position for preallocated arrays
    
    @elapsed for s_i in 1:n
        for a_i in 1:m
    
            # Store state and action indices
            s_indices[index] = s_i + 1
            a_indices[index] = a_i + 1
            
            # Placeholder for Q and R values (example, can be replaced with your actual logic)
            Q_vals[index] = 1.0  # You can replace 1.0 with actual values to be stored
            R_vals[index] = 1.0  # Same for R values
