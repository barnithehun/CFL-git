...
function parameters()
    ...
    return (rho_e = rho_e, sigma_e = sigma_e, nul_e = nul_e, alpha = alpha,
            ...
            kappa = kappa, zeta_R = zeta_R, zeta_L = zeta_L)
...
function FirmOptim(wage; phi_c)
    ...
    # calling parameters
    rho_e, sigma_e, nul_e, alpha, nu, pc, beta, delta, pdef_exo, discount, phi_a, tauchen_sd, kappa, zeta_R, zeta_L= parameters()

    # calling grid size
    x_size, e_size, k_size, b_size , t_size = gridsize()
    ...
    # setting optimization functions
        ...
        fn_Gam(k,val, Pi_liq, Pi_reo) = Int( phi_a*(1-delta)*k - Pi_liq >= phi_c*val - Pi_reo - zeta_R ) # liquidation decision b)

        function fn_Q(pdef, gam, Pi_liq, Pi_reo, next_b)
            if next_b == 0   
                q = 0.97
            else
                q = (beta / next_b) * ((1 - pdef) * next_b + pdef * min( next_b, gam * Pi_liq + (1 - gam) * Pi_reo ))
            end
        end
    ...
    # Setting the state-space
        ...
        t_grid = range(0, stop=1, length = t_size)
        ...
        m = k_size * b_size * t_size  # all possible actions
        ...
        # actions k,b,t, def, exit
        # do not need an extra action for reorg - enough to define that option in the return function
        a_vals = [gridmake(k_grid, b_grid, t_grid) zeros(m,2) ]
        a_vals = [a_vals; [0 0 0 1 0]; [0 0 0 0 1]]

        a_i_vals = [gridmake(1:k_size, 1:b_size, 1:t_size) zeros(m,2)]
        a_i_vals = Int.([a_i_vals; [1 div(k_size,2) 1 1 0]; [1 div(b_size,2) 1 0 1]])
    ...
    for a_i in 1:m
        for  s_i in 1:n 
            ...
            k = a_vals[a_i, 1]   
            b = a_vals[a_i, 2]   
            next_def = a_vals[a_i, 4] 
            next_exit = a_vals[a_i, 5] 
            # tau decision does not matter becuasue it does not affect x nor e transition        
    ...
    ################ 
        ...
        @elapsed for a_i in 1:m       
            for s_i in 1:n
                ...
                next_b = a_vals[a_i,2]
                next_t = a_vals[a_i,3]
                next_def =  a_i_vals[a_i,4]
                next_exit =  a_i_vals[a_i,5]

                gam =  gam_sa[s_i, a_i]
                pdef =  pdef_sa[s_i, a_i]
                Pi_liq = Pi_liq_sa[s_i, a_i]
                Pi_reo = Pi_reo_sa[s_i, a_i]

                if next_def == 0 && def == 0 && next_exit == 0
                        
                    # dividends
                    q = fn_Q(pdef, gam, Pi_liq, Pi_reo, next_b)
                ...
                # THIS WOULD CHANGE BECAUSE THE PAYOFF OF DEFAULTING CHANGES - IT BECOMES THE MAX OT THE TWO PAYOFFS
                elseif next_def == 1
                    R[s_i, a_i] = -5
                elseif next_exit == 1
                    R[s_i, a_i] = x  
        ...
        for s_i in 1:n
            ...
            # Summarise policies
            SumPol[s_i, :] .= [x, e, k, b, next_x, exit, def, pdef, q, l, y, Pi, d, gam, Pi_liq, Pi_reo, t, val]    

        end

        ###############################################################################
        # Pdef gam, PIliq and PIreo and implied q given optimal k', b' and t' in each state
        for s_i in 1:n
            ...
            for a_i in 1:m
                ...
                gam = 0
                Pi_reo = 0
                Pi_liq = max( 0, (1 - next_t) * phi_a*(1-delta)*next_k + next_t * kappa * phi_a*(1-delta)*next_k - zeta_L )
                ...
                for next_e_i in 1:e_size
                    ...
                    gam += p_trans * fn_Gam(next_k, val, Pi_liq, Pi_reo) 
                    Pi_reo += p_trans * ((1 - next_t) * phi_a*(1-delta)*next_k + next_t * phi_c*val)     
