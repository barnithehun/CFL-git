...
function parameters()
    ...
    return (rho_e = rho_e, sigma_e = sigma_e, nul_e = nul_e, alpha = alpha,
            ...
            kappa = kappa, zeta_L = zeta_L, tau_vec = tau_vec, phi_c = phi_c,
             phi_c_hh = phi_c_hh, Fcut = Fcut)
end

####### FIRM OPTIM #######
function FirmOptim(wage, zeta_Rl, shift, target_flag)

    rho_e, sigma_e, nul_e, alpha, nu, pc, beta, delta, pdef_exo_s, pdef_exo_l, discount, phi_a, phi_af, tauchen_sd, kappa,  zeta_L, tau_vec, phi_c, phi_c_hh, Fcut = parameters()
    ...
    while !isequal(kbexq_old,kbexq_new)
        ...
        for s_i in 1:n
            ...
            for a_i in 1:m
                ...
                if target_flag == 1 
                    phi_c_low = next_k < Fcut ? phi_c - shift : phi_c
                else
                    phi_c_low = phi_c - shift
                end
...
zeta_Rl = 2590
shift = 0
@elapsed SumPol, e_chain, Fmat = FirmOptim(wage, zeta_Rl, shift, 0)
c_e, f0 = EntryValue(SumPol, e_chain) ;
mu, m, xpol = stat_dist(SumPol, Fmat, f0);

zeta_Rl = 2590
shift = 0.1
@elapsed SumPol0, e_chain0, Fmat0 = FirmOptim(wage, zeta_Rl, shift, 0)
c_e0, f00 = EntryValue(SumPol0, e_chain0); 
mu0, m0, xpol0 = stat_dist(SumPol0, Fmat0, f0);

# The takeaway here is that fixed costs affect small firms, variable costs affect large firms
# In the assement of the SBRA, I could bring the example of large firms and a reduction of variable costs
...
######################## Targeted vs Universal policy effects - Shifting Costs ##########################
...
for (ind_s, zeta_Rs) in enumerate(zeta_Rs_vec)
    ...
    SumPol, e_chain, Fmat =  FirmOptim(wage, zeta_Rl, shift, 1)
    c_e, f0 = EntryValue(SumPol, e_chain) ;
    mu, m, xpol = stat_dist(SumPol, Fmat, f0);
    ...
    SumPol, e_chain, Fmat =  FirmOptim(wage, zeta_Rl, shift, 0)
    c_e, f0 = EntryValue(SumPol, e_chain) ;
    mu, m, xpol = stat_dist(SumPol, Fmat, f0);
