# Print policies
function PrintPolOld(SumPol, mu)    
    # PrintPol(SumPol, mu, SumPol0, mu0)    
    
        column_names = [:x, :e, :k, :b, :next_x, :exit, :def, :pdef, :q, :l, :y, :Pi, :d, :gam, :Pi_liq, :Pi_reo, :tau, :val, :SSperc, :SSpercCum]
    
        
        SumPoll = hcat(SumPol, round.(mu ./ sum(mu) .* 100, digits=4), cumsum(round.(mu ./ sum(mu) .* 100, digits=4)))
