...
master %>% filter(EASSTnum > 0) %>%  mutate(Less10m = ifelse(EASSTnum < 10^8, 1, 0)) %>% group_by(Less10m, chapter) %>% 
...
sum_master <- master %>% group_by(EASST, chapter) %>% 
  summarise(count = n(), totass = mean(EASSTnum)) %>% spread(chapter, count) %>% mutate(pr_liq = `7` / (`7`+`11`)) %>% arrange(totass)


write_csv(sum_master, "sum_master.csv")


sum_master$EASST <- factor(sum_master$EASST, levels = c("level1", "level2", "level3", "level4", "level5", "level6", 
                                                        "level7", "level8", "level9", "level10", "level11", "level12"))

p1 <- ggplot(master %>% filter(year(SNAPSHOT) >= 2010), aes(x = as.factor(EASSTnum), fill = as.factor(chapter))) +
  ...
  my_theme +
  theme(legend.position = c(0.8, 0.85),  # Adjust the x and y coordinates as needed
        legend.text = element_text(size = 12),
        legend.key.size = unit(1.4, "lines"))


p2 <- ggplot(sum_master, aes(x = as.factor(totass), y = pr_liq, group = 1)) +
  geom_line(color = "#377EB8", linewidth = 2, alpha = 1) +
