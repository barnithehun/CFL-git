...
\title{\Large{\textbf{Cash Flow-Based Lending, Bankruptcy\\ Resolution, and the Macroeconomy}}\thanks{ \href{https://github.com/barnithehun/Website/blob/gh/BSzekely_Draft.pdf}{Click here for the latest version} } }
\date{}

\begin{document}

\author{Barnabás Székely\thanks{\textbf{Institution:} Goethe Univesity, Frankfurt; \qquad \textbf{Email:} szekely.barnabas93@gmail.com;\\ \indent \  \hspace{0.8mm} \textbf{Website:} \url{https://www.barnabasszekely.com/}}}
\date{\today}
...
This paper argues that lacking the scale to reorganize under financial distress limits most firms' capacity to pledge future cash flows as collateral, with significant consequences for the broader economy. Given that liquidation wipes out future cash flows, lenders must compensate for liquidation risk by imposing higher spreads on cash flow-based debt. This elevates borrowing costs for small and medium-sized firms since the high costs of reorganization often force the liquidation of these businesses. I study the macroeconomic implications of this mechanism in a general equilibrium model, calibrated to the U.S. economy. I find that lower reorganization costs reduce ex-ante liquidation risk for smaller firms, allowing them to borrow against future cash flows at a lower price. The resulting improvement in credit allocation and firm entry raises aggregate productivity. These results underline the benefits of a reorganization-friendly bankruptcy regime. Size-based reforms, such as the U.S. Small Business Reorganization Act of 2019, can significantly raise aggregate productivity by improving small firms’ access to external finance.
...
Borrowing constraints defined by the value of firms' collateralizable assets create a self-perpetuating cycle: insufficient collateral limits access to external finance, which in turn limits growth and delays the accumulation of collateral. Accordingly, collateral constraints are often highlighted as major barriers to firm growth (Schmalz et al., 2017) and as key sources of credit misallocation and aggregate productivity losses (Catherine et al., 2021). Borrowing against future cash flows could, in principle, relax these constraints for productive but asset-poor firms, since this form of borrowing evaluates expected future earnings rather than the accumulated collateral (Lian and Ma, 2021). However, for most firms, cash flow-based borrowing remains significantly more expensive than borrowing against assets (asset-based debt).\footnote{US firms under 100M USD of assets pay an average credit spread of 7.19\% on cash flow-based debt and only 5.25\% on asset-based debt. See Panel A of \hyperref[tab:motivation]{{Table~\ref*{tab:motivation}}}.} This premium on CF-based debt may be detrimental to the broader economy, if it hinders the reallocation of capital to productive firms that could contribute the most to aggregate productivity (Li, 2022).

In this paper, I argue that a key factor contributing to the high cost of CF-based debt is the elevated liquidation risk associated with small and medium-sized firms. These businesses are significantly more likely to liquidate under financial distress than large enterprises, which reflects the administrative and time expenses of the reorganization process (Antill and Grenadier, 2019; Corbae and D'Erasmo, 2021).\footnote{57.9\% of US firms with assets under 100M USD liquidate in default, compared to just 5.7\% for larger firms. See Panel B of \hyperref[tab:motivation]{{Table~\ref*{tab:motivation}}}.} Given that liquidation wipes out all future cash flows, lenders must compensate for the elevated liquidation risk by imposing higher spreads on cash flow-based debt.\footnote{Lenders typically cannot overturn borrowers' liquidation decisions. I discuss this point further in section \ref{sec:emp liqprob}.} Hence, creditor perceptions of liquidation risk across different firms may be important determinant of credit spreads on CF-based debt. I study the broader macroeconomic implications of this mechanism in a heterogeneous firms model, calibrated to the U.S. economy. I ask: to what extent do reorganization costs raise liquidation risk, limit access to CF-based debt, and contribute to aggregate productivity losses?

To answer these questions, I develop a structural model that quantifies the general equilibrium effects of reorganization costs on liquidation probability and access to debt finance, along with macroeconomic consequences such as capital allocation and aggregate productivity. This exercise demands a rich model framework that incorporates in-equilibrium defaults and firm-specific external finance premia (Khan, Senga, and Thomas, 2017), bankruptcy resolution decisions (Tamayo, 2017; Antill and Grenadier, 2019; Corbae and D'Erasmo, 2021), and heterogeneity of debt contracts (Lian and Ma, 2021; Drechsel, 2023; Gonzalez and Sy, 2024). To the best of my knowledge, this is the first model to examine the macroeconomic effects of credit frictions in asset-based and cash flow-based debt financing, accounting for in-equilibrium defaults and heterogeneous bankruptcy resolution outcomes.

    In the model, firms finance their investments by net wealth and borrowing against assets or future cash flows. In each period, a positive share of firms experience financial distress in equilibrium, which leads to liquidation or reorganization, mirroring Chapter 7 and Chapter 11 of the U.S. bankruptcy code. Reorganization allows firms to continue operating but incurs fixed costs, which introduces economies of scale into bankruptcy resolution: large firms typically reorganize while smaller firms often liquidate. The lender sets firm-specific interest rates based on default risk. In case of cash flow-based lending, the lack of physical collateral increases the lender's exposure to liquidation risk. As a result, small firms that are expected to liquidate in default due to their size must pay higher spreads on CF-based debt to offset the elevated liquidation risk associated with them. This mechanism limits productive, but asset-poor firms' ability to pledge future cash flows as collateral, perpetuating credit misallocation. Moreover, it suppresses firm entry, as asset-poor entrants face high credits spreads and limited growth prospects. Together, these factors lead to a substantial loss in aggregate productivity.

The model is disciplined using a broad dataset of U.S. non-financial firms between 2010 and 2023. The calibrated model closely replicates key facts about US firms' debt financing strategies and default resolution decisions. To study the effects of reorganization costs on credit frictions and the broader economy, I estimate the effects of an exogenous shock that cuts the fixed costs of reorganization by half - this could be interpreted as a policy reform that implements a more reorganization-friendly bankruptcy regime. Small firms, who are disproportionately burdened by reorganization costs experience a large reduction in average liquidation probability from 80\% to 57\%, which fundamentally changes their optimal debt financing strategy. Their average reliance on CF-based debt jumps from 14\% to 50\% allowing them to rely on external finance more intensively. Their debt-to-collateral ratio rises by 4 percentage points, and the average interest rate falls by 0.65 percentage points. 
...
\end{equation}
and the ex-ante probability of liquidation under financial distress can be defined as:\footnote{The household's participation constraint $(1-\phi_c-\Pi_r(a,\varepsilon))V_{cont}(x,\varepsilon) - \zeta > 0$ always holds for or reorganizing firms under the current calibrations.} 
...
In February 2020, the Small Business Reorganization Act (SBRA) introduced Subchapter V to Chapter 11 of the U.S. Bankruptcy Code. This reform was based on the recognition that traditional Chapter 11 bankruptcy is often prohibitively expensive and time-consuming for small firms. The SBRA includes several provisions aimed at making the reorganization process more accessible for small businesses. \vspace{3mm} \\
\textit{1.) Elimination of Creditor Committees}. There is no automatic requirement to appoint a creditors' committee under Subchapter V, which reduces administrative costs and simplifies the bankruptcy process. \vspace{3mm} \\
...
\textit{3.) Modification of the Absolute Priority Rule}. Subject to court approval, Subchapter V allows firm owners to retain equity even if unsecured creditors are not fully repaid. This makes restructuring more viable for firms with limited collateral or high owner involvement. \vspace{3mm} \\
\textit{4.) Reduced Reporting and Disclosure Requirements}. Subchapter V imposes fewer financial reporting obligations on the debtor, which lowers compliance costs and streamlines the process. \vspace{3mm} \\
\textit{5.) Streamlined Reorganization Process}. The process includes shorter deadlines for filing and confirming a reorganization plan, thereby reducing both the time and cost involved in bankruptcy proceedings. \vspace{3mm} \\
...
In the following, it considers two aspects of the SBRA, that can be further studied using the structural model developed in the previous section. 
...
\begin{figure}[H]
    \centering
    \textbf{ Duration of Chapter 11 Cases / Million USD of Assets \vspace{2mm}} \\
    \includegraphics[width=0.8\textwidth]{C:/Users/szjud/OneDrive/Asztali gép/EBCs/CFL-git/Latex codes/Plots/daysin11.png} \\
...
\begin{figure}[H]  % [h] indicates placing the image here
    ...
    \includegraphics[width=1\textwidth]{C:/Users/szjud/OneDrive/Asztali gép/EBCs/CFL-git/Latex codes/Plots/large_reform.png}
