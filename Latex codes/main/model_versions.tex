...
\begin{itemize}\setlength\itemsep{0em} \small
    ...
    \item Find the inefficient liquidation vs. efficient reorganization quote  
    \item Read more on inefficient liquidation
\end{itemize} \normalsize

\subsubsection*{ToDo - Empirics}
\begin{itemize}\setlength\itemsep{0em} \small
    \item \textbf{Look into how you could retrieve liquidation probabilities (see C\&D)}
    \item Make a regression for the new outflows of debt
    \item Probably, it is better to have a Pols, Reduced Pols and FE  setup - the firms level regression is confusing
...
\begin{itemize}\setlength\itemsep{0em} \small
    \item Update default probabilities with the interpolated $x$-es
    \item Find a grid and calibration in ver5 where no firms are grid-constraint
    \item Figure out log-grids! 
    ...
    \item The big one: look into how entry and exit dynamics should be modeled in the discrete DP framework
    \item Take the the Quantecon examples and solve your model without global variables and with functions
\end{itemize} \normalsize
\textbf{Firm dynamics}: The way to think about it is probably as follows, what I have in ver6 describes firm decisions. That is, optimal $(k',b', def, exit)$. The exit and default decision shows the outflow of firms. The inflow of firms is given by the mass of the potential entrants, their distribution $(x, \varepsilon)$ and their decisions to enter given the value functions calculated above. So what you have to do probably is to save firm decisions, look at other models how they calculate the stationary distribution of firms and imitate that. Will not be able to use the built in version - there is a Hopenhayn model code in Julia so lets try to look at that.
\end{document}
