...
\begin{itemize}\setlength\itemsep{0em} \small
    ...
    \item Try implementing zero productivity in Q matrix instead of the exogenous defaults
    \item Rewrite $\phi_c$ in the liquidation function
    \item You can argue that you wanna give zero values to firms in default because if you do not then higher reorg. probability would affect firm values outside of reasons of debt finance. But, you want to shut down that channel
    \item Actually the model where households pay to keep firms alive seems quite realistic - could be sold instead of firms doing the same
    \item Think about making DRS intro a calibrated variable
\end{itemize} \normalsize
\newpage
