...
\begin{itemize}\setlength\itemsep{0em} \small
    ...
    \item Read Lian and Ma (2021) and results in Corbae and D'Erasmo (2021) and Drecshel microfundation 
    \item Re-Write empirics with the phrase: size is a virtue in itself when debt is secured against future cash flows. Obvious answer would be liquidation probability but there are naturally other alternative explanations for this.
    \item Info asymmetries must also play a role (that may be alleviated by size) but we also control for which probably accounts for much of the variation from info asymmetries. Relatedly credit ratings are also taken into account in the baseline regression. 
    \item Start re-writing the paper: see the annotated version of main 3. and the first paragraph in the 'model' notes. 
    \item Re-read Kochen results. 
...
\begin{itemize}\setlength\itemsep{0em} \small
    ...
    \item Check how well results work without taking credit ratings into acocount - what is the explanation? 
    \end{itemize} \normalsize
