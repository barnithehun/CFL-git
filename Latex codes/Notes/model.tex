...
Due to the exit dynamics only those firms exist in the market whose continuation value is larger than the liquidation value. This means that the liquidation probability is close to zero, CF-based debt is always preferred. \\
\newpage
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item \textbf{(!!!)} Read the paper on SBRA maybe there are some interesting facts in there that will give you some moments to match.
\end{itemize} \normalsize
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item \textbf{(!!!)} As presented now, the model is a bit of a black box. You need to be more explicit about the main model mechanism. Especially, the part on liquidation and reorganization must be more transparent - how is decision made? who gets what after liquidation and reorganization?
    \item \textbf{(!!!)} Say more about the resulting number of misallocation - how does it compare to the other, comparable models? (Especially Corbae and D'Erasmo would be relevant)
...
\begin{itemize}
    ...
    \item \textbf{(!!!)} It would be nice if you could find some empirical evidence that the recovery rates for CF-based debt in default is very low - \textit{EMphasize BWS, 2006. You should also have something to say that about debt that are backed by substantially all assets.}
    ...
    \item \textbf{(!!!)} Baseline results could be changed: instead of just reducing fixed costs, also increase the variable costs such that the average liquidation probability stays constant and the decreasing trend in liquidation probability is reduced - \textit{Should try this, it would solve the point saying `of course reducing cost would would imply better outcomes'.}
    \item \textbf{(!!!)} Maybe not just focus on SBRA but motivate by asking the effectiveness of 'size-based policies' or reducing the bargaining power of lenders - \textit{He seemed to think I could do better with the policy results.}
    ...
    \item \textbf{(!!!)} The steady state outcome should change a bit in the AB-case calibration because allowing more firms to survive through easier reorganizations Should allow more firms to survive - \textit{True, need to check the code} 
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item \textbf{(!!!)} Check the bankruptcy reform in Japan, in the 1990 crisis their issue was they could only liquidate, there is probably enough data too to do them - \textit{check their paper, they have an appendix on the Japan which probably helps.}
\end{itemize} \normalsize
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item \textbf{(!!!)} Check if the the pledgeability results holds if define pledgeability as ppentq / assets, also try the previous definition with che also in the pledgeability ration - \textit{if yes, rewrite the variable definition in the appendix because this is how you define the calibrated moment}
    ...
    \item \textbf{(!!!)} Check regression results with lags to avoid endogeneity issues - \textit{Should try. In any case, I should refer this idea in discussing the results, and the limitations of the empirical analyses part.} 
    \item \textbf{(!!!)} Rewrite the part on robustness checks, it refers to an older version of the paper, which I do not push anymore. 
\end{itemize} \normalsize
