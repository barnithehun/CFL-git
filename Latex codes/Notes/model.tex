...
\subsection*{Notes and Questions from Andrea Sy \checkmark} 
\begin{itemize} \setlength\itemsep{0em}  \small
...
\begin{itemize} \setlength\itemsep{0em}  \small
    \item Think about welfare - why could any policy could be welfare improving if there the decision to liquidation is made efficiently? - because lenders do not take into account that liquidation also limits access to CF-based debt
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item Could the lender get the cash flows even if the firm wants to liquidate? Would there be a scenario where the lender overturns the liquidation decision, or keeps the firm running after replacing the management.  \textit{It is possible but quite unlikely. The borrower is expected to file for bankruptcy, and chapter 7 filings almost never get turned into chapter 11. In the case of smaller firms, replacing the management also seems to costly.}
    \item Is the household a shareholder in the firm (owner)? Why would it be willing to pay for reorganization, you should check if the participation constraint of the household is satisfied. - \textit{The household is the owner of the firms hence it will always be willing to pay a fraction of the continuation value to the lender to keep the firm running.}
...
\begin{itemize} \setlength\itemsep{0em}  \small
    \item The empirical results are not specific to the process I am suggesting so do not push then - \textit{He is right.}
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item Is the paper a corporate finance or macro outcomes nad misallocation? 
    ...
    \item Is it impossible to raise equity in the model? - \textit{No. Which probably overemphasizes the effects of the core mechanism. But relatively smaller firms typically must heavily rely on debt finance anyways.}
...
\begin{itemize} \setlength\itemsep{0em}  \small
    \item Think more about the effects of information asymmetries. A lot of firms have uncertain or unpredictable cash flows which limit access to CF-based debt. How does this work in the model? Why do I not take this into account.
    ...
    \item As presented now, the model is a bit of a black box. You need to be more explicit about the main model mechanism. Especially, the part on liquidation and reorganization must be more transparent - how is decision made? who gets what after liquidation and reorganization?
    \item Say more about the resulting number of misallocation - how does it compare to the other, comparable models? (Especially Corbae and D'Erasmo would be relevant)
...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item The cost of seizing the assets argument: rather, you could say that when there is not physical collateral in place there is a cost of seizing the assets. You have to fight your ways to get it which might impose significant costs on the lender - might be able to tie employee compensations and administrative expenses of the bankruptcy proceeding having priority in this argument. The point here is to come up with a good story not really that you do anything much better. 
    \item Can it happen that a AB lender gets reorganized in the end? \textit{Yes it could, but it is quite unlikely.}
    ...
    \item My notes: In the model asset based lending and CF-based lending can exceed to value of physical collateral or the value of CF-s. In the case of AB debt this is a problem, since if the value of the debt exceeds the value of physical collateral then conventionally you would call that CF-debt. \textit{I could come up with a setup where this is not the case and asset-based debt is always paid in full, and there is no variations in the spreads of AB-debt empirically. Although this does not seem to be the case - I did not consider this as it would only be added complication although it would be more realistic. If I took this into account the estimated effect would be even stronger, since small firms, that would like to borrow in excess their of their liquidation value would be forced to borrow against CFs which is then affected by liquidation probability. I think the reasoning that I want to make this paper about liquidation probability not debt capacity is strong enough.}
\end{itemize} \normalsize

\subsection*{Notes and Questions from Priit Jenas}
\begin{itemize}
    \item Read "Leverage and Productivity" - Huiyu Li, 2022
    \item Read "Financial Frictions and Firm Exit" - Bornstein and Castillo-Martinez
    \item It would be nice if you could find some empirical evidence that the recovery rates for CF-based debt in default is very low - \textit{EMphasize BWS, 2006. You should also have something to say that about debt that are backed by substantially all assets.}
    \item How often do endogenous default happen - should be almost never right -\textit{True, it only happens for very young firms that are in the process of borrowing a lot to scale up. This is why I also need exogenous default.}
    \item Baseline results could be changed: instead of just reducing fixed costs, also increase the variable costs such that the average liquidation probability stays constant and the decreasing trend in liquidation probability is reduced - \textit{Should try this, it could solve the point that of course reducing cost would would imply better outcomes.}
    \item Maybe not just focus on SBRA but motivate by asking the effectiveness of 'size-based policies' or reducing the bargaining power of lenders - \textit{He seemed to think I could do better with the policy results.}
    \item If you had intangible assets or another productive factor of production that needs investment, over-investment in capital would easier - \textit{Not sure what to make of this comment.} 
    \item The steady state outcome should change a bit in the AB-case calibration because allowing more firms to survive through easier reorganizations Should allow more firms to survive - \textit{True, need to check the code} 
    \item He also would like to see misallocation decomposition - \textit{Something to do later.}
\end{itemize}


\subsection*{Chen Lian Notes and Questions}
\begin{itemize} \setlength\itemsep{0em}  \small
    \item Motivation is about misallocation but the result is really not. Need to do more results on the macro front - \textit{Do the decomposition of effects.}
    \item Microfoundation of the model works - \textit{Great news.}
    \item Check the bankruptcy reform in Japan, in the 1990 crisis their issue was they could only liquidate, there is probably enough data too to do them - \textit{check their paper, they have an appendix on the Japan which probably helps.}
\end{itemize} \normalsize


\subsection*{Aux Notes and Questions}
\begin{itemize} \setlength\itemsep{0em}  \small
    \item Why do lenders have a near-zero payoff if they hold CF-based debt, but the borrower liquidates? - \textit{Technically, the lender could take unencumbered assets, but this is not likely to be the case for most firms. In Compustat, 53\% of firms hold both type of debt simultaneously. Even, if  the creditor often cannot make sure that the firm will not take higher-priority debt later - Ivashina and Vallée (2025), argues that non-pledge covenants are frequently undermined by 	contractual loopholes. Accordingly, Bris et al. (2006) finds that in 95\% of chapter 7 cases, the unsecured lender retrieves nothing.}
    \item Are there any cases where the lender would prefer reorganization but the firm is liquidated - \textit{Yes, it happens a due to the fixed costs of reorganization which is not internalized by the lender.}
    \item In the paper you have to clearly differentiate between expected payments and actual payoffs are they the same- \textit{!!!}, maybe assume that they end up the same
    \item Make a 75 mins version of you paper! You need to fix the description of the defaults ($\phi_c$ is not correct.) You should have a much better strategy talking about these - \textit{!!!} 
    \item Also a better explanation of what happens in reorg. and liquidation and all the four pieces of the table
    \item You should have something about capital allocation pre and post reform 
    \item How does the firm continue after reorganization? - \textit{The firm is reorganized withing the period, continues with the same productivity and cash on hand and can chose next period capital and debt as if nothing happened. The debt contract that is for the current period is washed away. This is because the household paid the lender to keep the firm running.}
    \item How is debt to collateral calculated in the model? - \textit{Future debt over future capital in the stationary equilibrium. Note that in the stationary equilibrium the timing of the averages does not matter. Moreover, it is not k+x because the lender seize cash on hand in case of liquidation - check in the model what happens to cash on hand after default, it should be a DWL or reallocated to households (Jermann and Quadrini, 2012)}
    \item So asset based debt is always fully collateralized? - \texit{In the model, I define AB, CF debt differently. AB-debt is where debt is backed by liquidation value. CF-based debt is where debt is backed by future cash flows. This allows AB-debt to not be fully collateralized. This definition does not exactly mirror who we see AB-CF debt in reality, where the under-collateralized part of the AB debt is technically CF-based. However, this would introduce a lot of messyness.}
    \item How is CF-based lending different from equity financing, 
\end{itemize} \normalsize
