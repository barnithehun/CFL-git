...
This is not a proble because reorganization happens within the period, so the frim may continue operating as if no default occured. No need to treat reorganizing firms any differently from liquidating firms and you do not need to take into account reorganizing firms when you calculate exiting firms. \vspace{3mm} \\
...
\end{aligned} $$
This conditon implies that for each firm, capital to labour ratio is constant $\phi$, 
...
Dividing the two equations abouve implies that the efficient allocation is: 
...
\small
\begin{itemize}
    \item My job market paper studies how small and medium-sized firms' tendency to liquidate in default limits their ability to borrow against future cash flows (earnings).
    \item The traditional view is that the main reason why small firms are credit-constrained is the lack of physical collateral.
    \item This view contradicts recent evidence suggesting that most corporate debt is backed by future cash flows rather than physical assets.
    \item I suggest that the limiting factor in small firms' access to debt is high ex-ante liquidation risk.
    \item I build a quantitative model around this. Heterogeneous firms are subject to idiosyncratic productivity shocks which may yield default, which triggers bankruptcy resolution procedures.
    \item Firm may be liquidated which implies exit from the market, or reorganized which allows them to continue operating but tis subject to a fixed cost.
    \item This fixed costs reflects the observation that reorganization is a time and resource intensive process, and small firms often lack the scale to justify these expenses. 
    \item Lender redistribute household savings to firms subject to a zero-profit condition. They may lend against assets and cash flows. Cash flow based debt is exposed to liquidation risk since liquidation terminates all future cash flows. Hence, lenders charge a higher interest rate to firms with a high liquidation probability.
    \item As a result, asset-poor firms are simultaneously constrained by a lack of physical collateral and high liquidation risk, leading to capital misallocation, subdued firm entry and aggregate productivity losses.
    \item To quantify the importance of this mechanism, I calibrate the model to U.S. corporations and study the effects of an exogenous decrease in reorganization costs.
    \item I find that lowering reorganization costs significantly increases small firms' reliance on cash flow-based lending, narrows the financing gap between small and large firms, and raises aggregate productivity by promoting firm entry and improving capital allocation.
    \item Reducing reorganization costs may seem like a purely academic exercise, but it has clear policy relevance.
    \item It highlights how reorganization friendly bankruptcy procedures can improve financing for small and medium-sized enterprises.
    \item The US has recently introduced the SBRA to streamline bankruptcy procedures for small businesses, and similar reforms have implemented in the EU reorganization directive. These aim to streamline the reorganization process and reduce associated costs to reduce inefficient liquidations. The SBRA is size-based the EU directive is not. 
\end{itemize} \normalsize
...
\begin{itemize} \setlength\itemsep{0em}  \small
    \item \textbf{(!)} Need a better intro. The current one is `too nerdy'. 
    \begin{itemize}
        \item Maybe start with the policy motivation first (Georg) or with arrow describing the mechanism (Lidia).
        \item Would the law apply for firms under 100 million USD in assets? If not why choose this cutoff? - \textit{Originally this cutoff was intended to show that the premium for CF-based debt extends beyond SMEs. But I see how it is confusing in relation to the SBRA.}
        \item \textbf{(!)} Try to avoid corporations vs. firms - stick to firms. 
        \end{itemize}
    \item \textbf{(!)} DO NOT tell people what you tried to do and failed. They are not interested. 
    \item \textbf{(?)} Are the macroeconomic effects facts, assumptions or model predictions? 
    ...
    \item \textbf{(!)} Does the model looks any transmission dynamics? - \textit{No, I just compare stationary equilibria before and after the reform. The problem is that I calibrate to a period where the already existed. It would be cleaner to calibrate the model to 2010-2020. This is true, but I not sure recalibrating the model is worth the effort at this point.}
    \item Highlight the difference between exit and default better
    ...
    \item \textbf{(!)} Color code the boxed that you talk about 
    \item \textbf{(!)} Even with the variable costs, as long as they only depend on assets, you could get this decreasing reorganization cost relative to firms size. - \textit{Since there are no adjustment costs, capital and productivity are pretty highly correlated in the model, so there this is not true. Empirically, fixed costs are quite well established (Wang, 2022; Corbae and D'Erasmo, 2021).}
    ...
    \item \textbf{(?)} Firms can only have one type of debt in the model? - \textit{In Compustat, 53\% of firms have both types of debt simultaneously. My model allows for this, but firms optimally choose only one type of debt. This is a more general problem with the literature.}
    \item \textbf{(!)} Move entry cost to the the internally calibrated parameters.
    ...
    \item \textbf{(?)} Large firms cannot really much more on CF-based debt in the model but in reality they can - \textit{Look at the interest rate margina as well. Even if they would rely a 100 on CF-based debt, they could achive gains from accessing it cheaper.}
    \item \textbf{(!)} Firm mass and mean firms size should be expressed as changes (add a change column to the results). 
    ...
    \noindent \textbf{Notes after the speech.}
    \item \textbf{(!)} Be more enthusiastic in the presentation.
    \item \textbf{(!)} Too many words on the slides 
    \item \textbf{(!)} Think why any of this is imporant? Big picture implications, sort of.
    \item \textbf{(!)} Do not answer questions with `thats a good comment'.
    \item \textbf{(!)} You say liquidation probability but that implies exogeneity. You should say that small firms tendency to liquidate in defualt or smth. 
    \item \textbf{(!)} Silde 7, the model summary: one should be able to see more of the model mechanism and tradeoffs already.
    \item \textbf{(!)} Define small firms better - they could be smaller firms or something else.
    \item \textbf{(!)} Be more precise. 
    \item \textbf{(!)} Do not need slide 3 (?)
    \item 

\end{itemize} \normalsize


\subsection*{Aux Notes and Questions}
\begin{itemize} \setlength\itemsep{0em}  \small
    \item \textbf{(?)} Why do lenders have a near-zero payoff if they hold CF-based debt, but the borrower liquidates? - \textit{Technically, the lender could take unencumbered assets, but this is not likely to be the case for most firms. In Compustat, 53\% of firms hold both type of debt simultaneously. Even, if  the creditor often cannot make sure that the firm will not take higher-priority debt later - Ivashina and Vallée (2025), argues that non-pledge covenants are frequently undermined by 	contractual loopholes. Accordingly, Bris et al. (2006) finds that in 95\% of chapter 7 cases, the unsecured lender retrieves nothing.}
    \item \textbf{(?)} Are there any cases where the lender would prefer reorganization but the firm is liquidated - \textit{Yes, it happens a due to the fixed costs of reorganization which is not imposed on the lender.}
    ...
    \item \textbf{(?)} How does the firm continue after reorganization? - \textit{The firm is reorganized within the period, continues with the same productivity and cash on hand and can chose next period capital and debt as if nothing happened. The debt contract that is for the current period is washed away. This is because the household paid the lender to keep the firm running.}
    \item \textbf{(?)} How is debt to collateral calculated in the model? - \textit{Future debt over future capital in the stationary equilibrium. Note that in the stationary equilibrium the timing of the averages does not matter. Moreover, it is not k+x because the lender seize cash on hand in case of liquidation - check in the model what happens to cash on hand after default, it should be a DWL or reallocated to households (Jermann and Quadrini, 2012)}
    \item  \textbf{(?)} So asset based debt is always fully collateralized? - \textit{In the model, I define AB, CF debt differently how they are commonly referred to. Following Lian and Ma (2021), AB-debt is where debt is backed by liquidation value. CF-based debt is where debt is backed by future cash flows. This allows AB-debt to not be fully collateralized. This is where it is important that this distiction is different from secured and unsecured debt.}
    \item  \textbf{(?)} In the model asset based lending and CF-based lending can exceed to value of physical collateral or the value of CF-s. In the case of AB debt this is a problem, since if the value of the debt exceeds the value of physical collateral then conventionally you would call that CF-debt. \textit{I could come up with a setup where this is not the case and asset-based debt is always paid in full, and there is no variations in the spreads of AB-debt empirically. Although this does not seem to be the case - I did not consider this as it would only be added complication although it would be more realistic. If I took this into account the estimated effect would be even stronger, since small firms, that would like to borrow in excess their of their liquidation value would be forced to borrow against CFs which is then affected by liquidation probability. I think the reasoning that I want to make this paper about liquidation probability not debt capacity is strong enough.}
    \item \textbf{(?)} How is CF-based lending different from equity financing? - \textit{The lender does not have a claim on the firms' shares. The value of future cash flow only comes into play when the firm is under financial distress. The lenders' payoff from the debt is bounded from above, by the original value of the debt times risk-free rate. } 
