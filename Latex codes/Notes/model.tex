...
\begin{itemize} \setlength\itemsep{0em}  \small
    ...
    \item Are there any cases where the lender would prefer reorganization but the firm is liquidated - \textit{Yes, it happens a due to the fixed costs of reorganization which is not imposed on the lender.}
    ...
    \item  \textbf{(?)} So asset based debt is always fully collateralized? - \textit{In the model, I define AB, CF debt differently how they are commonly referred to. Following Lian and Ma (2021), AB-debt is where debt is backed by liquidation value. CF-based debt is where debt is backed by future cash flows. This allows AB-debt to not be fully collateralized. This definition does not exactly mirror who we see AB-CF debt in reality, where the under-collateralized part of the AB debt is technically CF-based. However, this would introduce a lot of messyness.}
